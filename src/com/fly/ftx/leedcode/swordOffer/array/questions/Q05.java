...
public class Q05 {
    ...
    public static String replaceSpace(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if(charAt == ' ') str.append("%20");
            else str.append(charAt);
        }
        return str.toString();
    }
     /**
     * 思路：
     *  将字符串转换成字符数组进行操作，因为字符串底层存储数据结构就是字符数组，然后操作字符数组还是很轻松的。
     * 难点：
    ...
    public static String replaceSpace01(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if(charArray[i] == ' ') str.append("%20");
            else str.append(charArray[i]);
        }
        return str.toString();
    }
